/**
 *    Filename: ConversionDialog.java
 *     Version: 0.9.0
 * Description: Conversion Dialog
 *     License: GPLv2
 *        Date: 2017-01-14
 *
 * @author unknown
...
public class ConversionDialog extends JDialog implements ActionListener, FocusListener, KeyListener {
		...
		try {
			...
			// Pressure
			{
				...
				for (int i = 0; i < pres.length; i++) {
					lPres[i] = new JLabel(StringUtils.capitalize(pres[i].unit));
			...
			// Vol
			{
				...
				for (int i = 0; i < vols.length; i++) {
					lVols[i] = new JLabel(StringUtils.capitalize(vols[i].unit));
			...
			// Weight
			{
				...
				for (int i = 0; i < weights.length; i++) {
					lWeights[i] = new JLabel(StringUtils.capitalize(weights[i].unit));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o == textF ) {
			...
			textC.setText(StringUtils.format(BrewCalcs.fToC(f), 1));
		} else if (o == textC ) {
			double c = Double.parseDouble(textC.getText());
			textF.setText(StringUtils.format(BrewCalcs.cToF(c), 1));
		} else if (o == textSG ) {
			double sg = Double.parseDouble(textSG.getText());
			textPlato.setText(StringUtils.format(BrewCalcs.SGToPlato(sg), 2));
			textGU.setText(StringUtils.format(BrewCalcs.sgToGU(sg), 0));
		} else if (o == textPlato ) {
			double plato = Double.parseDouble(textPlato.getText());
			double sg = BrewCalcs.platoToSG(plato);
			textSG.setText(StringUtils.format(sg, 3));
			textGU.setText(StringUtils.format(BrewCalcs.sgToGU(sg), 0));
		} else if (o == textGU ) {
				...
				if (o == textPres[i]) {
					...
					for (int j = 0; j < textPres.length; j++) {
						if (i != j) {
							...
							textPres[j].setText(StringUtils.format(conv, 2));
						}
					}
					return;
				}
				...
				if (o == textVols[i]) {
					...
					for (int j = 0; j < textVols.length; j++) {
						if (i != j) {
							...
							textVols[j].setText(StringUtils.format(conv, 2));
						}
					}
					return;
				}
				...
				if (o == textWeights[i]) {
					...
					for (int j = 0; j < textWeights.length; j++) {
						if (i != j) {
							...
							textWeights[j].setText(StringUtils.format(conv, 2));
						}
					}
					return;
				}
