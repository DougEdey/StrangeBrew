...
public class Mash {
	...
	private String getVolConverted(double val){
		...
		String s = StringUtils.format(d, 1);
		return s;
	}
	...
	public String getMashTotalVol() {
		...
		String s = StringUtils.format(d, 1) + " " + volUnits;
		return s;
	}
	...
	public void calcMashSchedule() {
		...
		stp.directions = "Mash in with " + StringUtils.format(stp.inVol.getValueAs(volUnits),1 ) + " " + volUnits
				+ " of water at " + StringUtils.format(strikeTemp, 1) + " " + tempUnits;
		...
		for (int i = 1; i < steps.size(); i++) {
			...
			if (stp.method.equals(INFUSION)) { // calculate an infusion step
				...
				if (tempUnits == "C")
					strikeTemp = 100;
				stp.directions = "Add " + StringUtils.format(stp.inVol.getValueAs(volUnits), 1) + " " + volUnits
						+ " of water at " + StringUtils.format(strikeTemp, 1) + " " + tempUnits;
				...
				stp.directions = "Remove " + StringUtils.format(stp.outVol.getValueAs(volUnits), 1) + " " + volUnits
				...
				if (newTemp < targetTemp){
					...
					addStr = " Add " + StringUtils.format(Quantity.convertUnit("qt", volUnits, extraWaterQTS), 1)
				...
				String weightStr = StringUtils.format(Quantity.convertUnit(Quantity.LB, myRecipe.getMaltUnits(), stp.weightLbs), 1)
				+ " " + myRecipe.getMaltUnits();
				String volStr = StringUtils.format(Quantity.convertUnit(Quantity.QT, volUnits, waterAddedQTS), 1)
				+ " " + volUnits;
				if (tempUnits == "C"){
					strikeTemp = BrewCalcs.fToC(strikeTemp);
				...
				String tempStr = StringUtils.format(strikeTemp, 1) + tempUnits;
				String tempStr2 = StringUtils.format(cerealTemp, 1) + tempUnits;
				String tempStr3 = StringUtils.format(targetTemp, 1) + tempUnits;
				String tempStr4 = StringUtils.format(cerealTargTemp, 1) + tempUnits;
       ...
       // do we need any more steps?
		...
		for (int i = 1; i < steps.size(); i++) {
			...
			if (stp.getType().equals(SPARGE)) {
				...
				String collectStr = StringUtils.format(Quantity.convertUnit(Quantity.QT, volUnits, collect[j]), 2) +
				" " + volUnits;
				String tempStr;
				if (tempUnits.equals("F")){
					tempStr = "" + StringUtils.format(SPARGETMPF, 1) + "F";
				} else {
					tempStr = StringUtils.format(BrewCalcs.fToC(SPARGETMPF), 1) + "C";
				}

				if (numSparge > 1){
					stp.setMethod(BATCH);
					String add = StringUtils.format(Quantity.convertUnit(Quantity.QT, volUnits, charge[j]), 2) +
					" " + volUnits;
					...
					stp.setDirections("Sparge with " +
							StringUtils.format(Quantity.convertUnit("qt", volUnits, spargeQTS), 1) +
	...
	double calcWaterAddition(double targetTemp, double currentTemp,
	 ...
	 public String toXml() {
		...
		sb.append("  <MASH>\n");
		sb.append(StringUtils.xmlElement("NAME", name, 4));
		sb.append(StringUtils.xmlElement("MASH_VOLUME", StringUtils.format(Quantity.convertUnit("qt", volUnits, volQts), 2) , 4));
		sb.append(StringUtils.xmlElement("MASH_VOL_U", "" + volUnits, 4));
		sb.append(StringUtils.xmlElement("MASH_RATIO", "" + mashRatio, 4));
		sb.append(StringUtils.xmlElement("MASH_RATIO_U", "" + mashRatioU, 4));
		sb.append(StringUtils.xmlElement("MASH_TIME", "" + totalTime, 4));
		sb.append(StringUtils.xmlElement("MASH_TMP_U", "" + tempUnits, 4));
		sb.append(StringUtils.xmlElement("THICK_DECOCT_RATIO", "" + thickDecoctRatio, 4));
		sb.append(StringUtils.xmlElement("THIN_DECOCT_RATIO", "" + thinDecoctRatio, 4));
		if (tempUnits.equals("C")){
			sb.append(StringUtils.xmlElement("MASH_TUNLOSS_TEMP", "" + (tunLossF/1.8), 4));
			sb.append(StringUtils.xmlElement("GRAIN_TEMP", "" + BrewCalcs.fToC(grainTempF), 4));
			sb.append(StringUtils.xmlElement("BOIL_TEMP", "" + BrewCalcs.fToC(boilTempF), 4));
		}
		else {
			sb.append(StringUtils.xmlElement("MASH_TUNLOSS_TEMP", "" + tunLossF, 4));
			sb.append(StringUtils.xmlElement("GRAIN_TEMP", "" + grainTempF, 4));
			sb.append(StringUtils.xmlElement("BOIL_TEMP", "" + boilTempF, 4));
		}
		for (int i = 0; i < steps.size(); i++) {
			...
			if (tempUnits.equals("C"))
				sb.append("      <DISPL_TEMP>" + StringUtils.format(BrewCalcs.fToC(st.startTemp), 1) + "</DISPL_TEMP>\n");
			else
				sb.append("      <DISPL_TEMP>" + st.startTemp + "</DISPL_TEMP>\n");
			sb.append("      <END_TEMP>" + st.endTemp + "</END_TEMP>\n");
			if (tempUnits.equals("C"))
				sb.append("      <DISPL_END_TEMP>" + StringUtils.format(BrewCalcs.fToC(st.endTemp), 1) + "</DISPL_END_TEMP>\n");
			else
